...
interface HotelRepository extends Repository<Hotel, Long> {
	...
	@Query("select new org.asgs.lms.data.jpa.domain.HotelSummary(h.city, h.name, avg(r.rating)) "
			+ "from Hotel h left outer join h.reviews r where h.city = ?1 group by h")
	Page<HotelSummary> findByCity(City city, Pageable pageable);

	@Query("select new org.asgs.lms.data.jpa.domain.RatingCount(r.rating, count(r)) "
